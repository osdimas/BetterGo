...
def aimbot():
    ...
    while True:
        try:
            if ctypes.windll.user32.GetAsyncKeyState(key_handler('aimbot_key')) and dpg.get_value(
                    'aimbot_checkbox') and ent.in_game():
                ...
                if best_angle.x < fov and best_angle.y < fov and best_angle.x != 0.0 and best_angle.y != 0.0:
                    if dpg.get_value('aimbot_smooth') < 1.0:
                        ent.set_view_angle(Vector3(view_angle.x + best_angle.x - aim_punch.x if dpg.get_value(
                            'aimbot_rcs_checkbox') else view_angle.x + best_angle.x,
                                                   view_angle.y + best_angle.y - aim_punch.y if dpg.get_value(
                                                       'aimbot_rcs_checkbox') else view_angle.y + best_angle.y,
                                                   view_angle.z + best_angle.z - aim_punch.z if dpg.get_value(
                                                       'aimbot_rcs_checkbox') else view_angle.z + best_angle.z
                                                   ))
                    else:
                        ent.set_view_angle(Vector3(view_angle.x + (best_angle.x - aim_punch.x) / dpg.get_value(
                            'aimbot_smooth') if dpg.get_value(
                            'aimbot_rcs_checkbox') else view_angle.x + best_angle.x / dpg.get_value('aimbot_smooth'),
                                                   view_angle.y + (best_angle.y - aim_punch.y) / dpg.get_value(
                                                       'aimbot_smooth') if dpg.get_value(
                                                       'aimbot_rcs_checkbox') else view_angle.y + best_angle.y / dpg.get_value(
                                                       'aimbot_smooth'),
                                                   view_angle.z + (best_angle.z - aim_punch.z) / dpg.get_value(
                                                       'aimbot_smooth') if dpg.get_value(
                                                       'aimbot_rcs_checkbox') else view_angle.z + best_angle.z / dpg.get_value(
                                                       'aimbot_smooth')
                                                   ))
        except Exception as err:
            pass
    ...
    while True:
        try:
            if dpg.get_value('player_esp'):
                ...
                c1 = [round(enemy_team_color[0], 1), round(enemy_team_color[1], 1), round(enemy_team_color[2], 1),
                      round(enemy_team_color[3], 1)]
                c2 = [round(team_color[0], 1), round(team_color[1], 1), round(team_color[2], 1),
                      round(team_color[3], 1)]
                ...
                for entity in ent.glow_objects_list:
                    if entity[2] == 40:
                        ...
                        if ent.get_dormant(entity[1]) == True:
                            continue
                        if ent.get_team(entity[1]) != ent.get_team(lp.local_player()):
                            game_handle.write_float(ent.glow_object() + ((0x38 * (entity[0] - 1)) + 0x8),
                                                    c1[0] / 255 if not dpg.get_value('health_mode_checkbox') else (
                                                                                                                              255.0 - 2.55 * ent.get_health(
                                                                                                                          entity[
                                                                                                                              1])) / 255.0)
                            game_handle.write_float(ent.glow_object() + ((0x38 * (entity[0] - 1)) + 0xC),
                                                    c1[1] / 255 if not dpg.get_value('health_mode_checkbox') else (
                                                                                                                              2.55 * ent.get_health(
                                                                                                                          entity[
                                                                                                                              1])) / 255.0)
                            game_handle.write_float(ent.glow_object() + ((0x38 * (entity[0] - 1)) + 0x10),
                                                    c1[2] / 255 if not dpg.get_value('health_mode_checkbox') else 0.0)
                            ...
                            game_handle.write_float(ent.glow_object() + ((0x38 * (entity[0] - 1)) + 0x8),
                                                    c2[0] / 255 if not dpg.get_value('health_mode_checkbox') else (
                                                                                                                              255.0 - 2.55 * ent.get_health(
                                                                                                                          entity[
                                                                                                                              1])) / 255.0)
                            game_handle.write_float(ent.glow_object() + ((0x38 * (entity[0] - 1)) + 0xC),
                                                    c2[1] / 255 if not dpg.get_value('health_mode_checkbox') else (
                                                                                                                              2.55 * ent.get_health(
                                                                                                                          entity[
                                                                                                                              1])) / 255.0)
                            game_handle.write_float(ent.glow_object() + ((0x38 * (entity[0] - 1)) + 0x10),
                                                    c2[2] / 255 if not dpg.get_value('health_mode_checkbox') else 0.0)
    ...
    while (True):
        try:
            if dpg.get_value('standalone_rcs_checkbox') and ent.in_game() and ent.get_health(lp.local_player()) > 0:
                if ctypes.windll.user32.GetAsyncKeyState(key) and ent.get_shots_fired() > dpg.get_value(
                        'rcs_get_bullets'):
                    if weapon_rifle(lp.active_weapon()) or weapon_smg(lp.active_weapon()) or weapon_heavy(
                            lp.active_weapon()):
                        view_angle = ent.get_view_angle()
    ...
    while True:
        try:
            if dpg.get_value('autopistol_checkbox') and ent.in_game():
                if ctypes.windll.user32.GetAsyncKeyState(key_handler('autopistol_key')) and weapon_pistol(
                        lp.active_weapon()):
                    lp.force_attack(6)
                    time.sleep(0.02)
        except Exception as err:
    ...
    while True:
        try:
            if ent.in_game():
                ...
                if ctypes.windll.user32.GetAsyncKeyState(key_handler('triggerbot_key')) and dpg.get_value(
                        'triggerbot_checkbox'):
                    if lp.get_team_by_crosshair(entity) != ent.get_team(
                            lp.local_player()) and lp.get_health_by_crosshair(entity) >= 1:
                ...
                if dpg.get_value('knifebot_checkbox'):
                    if weapon_knife(lp.active_weapon()) and lp.get_team_by_crosshair(entity) != ent.get_team(
                            lp.local_player()):
                        if distance <= 82:
...
#     while True:
#         try:
#             if ctypes.windll.user32.GetAsyncKeyState(0x20) and dpg.get_value('bunnyhop_checkbox') and dpg.get_value(
#                     'auto_strafer_checkbox'):
            ...
            if ent.in_game():
                ...
                for entity in ent.entity_list:
                    if entity[2] == 40:
                        ...
                        if ent.get_team(entity[1]) == ent.get_team(lp.local_player()):
                            observed_target_handle = game_handle.read_uint(
                                entity[1] + offsets.m_hObserverTarget) & 0xFFF
                            spectated = game_handle.read_uint(
                                mem.client_dll + offsets.dwEntityList + (observed_target_handle - 1) * 0x10)
                ...
                for entity in ent.entity_list:
                    if entity[2] == 40:
                        if ent.get_name(entity[0]) not in [None, 'GOTV']:
                            ...
                            execute_cmd(
                                f'echo {entity[0]} {name} {ent.get_wins(entity[0])} {ranks_list[ent.get_rank(entity[0])]}'.encode(
                                    'ascii'), (engine_dll + offsets.Cmd_ExecuteCommand))
                            time.sleep(0.1)
        except Exception as err:
            pass
        time.sleep(0.001)
...
#                             game_handle.write_float(entity[1] + offsets.m_flCustomAutoExposureMin,
#                                                     dpg.get_value('nightmode_strength'))
#                             game_handle.write_float(entity[1] + offsets.m_flCustomAutoExposureMax,
#                                                     dpg.get_value('nightmode_strength'))
